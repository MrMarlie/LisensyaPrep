...
export default function AnnouncePNLEMockButton() {
  ...
  async function handleSend() {
    ...
    setResult('');
    try {
      const res = await fetch('/api/admin/announce-pnle-mock', { method: 'POST' });
  ...
  return (
    <div className="bg-[#0f1629] border border-pink-400/30 rounded-xl p-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-pink-400 font-bold text-sm mb-1">📢 Announce PNLE Mock Board (₱99)</p>
          <p className="text-gray-500 text-xs">
            Emails every PNLE Nursing Starter Pack subscriber that the ₱99 PNLE Mock Board is live.
          </p>
          {result && (
          ...
          {confirmed && status === 'idle' && (
            <p className="text-pink-400 text-xs mt-2 font-semibold">
              ⚠️ Click again to confirm — this emails all PNLE Starter Pack subscribers.
            </p>
          )}
        </div>
        <button
