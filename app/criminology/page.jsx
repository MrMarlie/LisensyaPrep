...
const EXAM_INFO = [
  { label: 'Board Exam Name', value: 'Criminologist Licensure Examination (CLE)' },
  { label: 'Conducted By', value: 'Professional Regulation Commission (PRC)' },
  { label: 'Schedule', value: 'Twice a year (usually August and March)' },
  { label: 'Passing Rate', value: '75% general weighted average required' },
  { label: 'Subjects Covered', value: 'Criminal Jurisprudence, Criminalistics, Law Enforcement, Crime Investigation, Corrections, Sociology of Crimes' },
];
...
export default function CriminologyPage() {
  const modules = Object.values(MODULE_INFO).filter((m) => m.examId === 'criminology');

  return (
    <div className="min-h-screen">
      {/* Hero */}
      <section className="relative bg-gradient-to-b from-red-950/40 to-[#080d1b] py-16 sm:py-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-gray-300 text-sm">Criminology</span>
          </div>

          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/30 text-red-400 text-sm font-semibold px-4 py-2 rounded-full mb-5">
              ⚖️ Criminologist PRC Licensure Exam
            </div>
            <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4 leading-tight">
              Criminology PRC
              <br />
              <span className="text-red-400">Licensure Exam Review</span>
            </h1>
            <p className="text-gray-400 text-lg mb-8 max-w-2xl leading-relaxed">
              Battle through 6 modules covering the complete Criminologist Licensure Examination syllabus.
              Defeat the exam boss and collect your PRZ piece for each stage you complete.
            </p>

            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-red-400">✓</span> 6 Modules
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-red-400">✓</span> 300 Questions Total
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-red-400">✓</span> Detailed Explanations
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-red-400">✓</span> Progress Saved
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-14 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((mod) => (
            <Link key={mod.id} href={`/criminology/${mod.id}`} className="group">
      ...
      <section className="py-12 bg-[#0a1029]/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-extrabold text-white mb-6">About the Criminologist PRC Board Exam</h2>
