...
export default function CLEMasteryCheckoutPage() {
  const router = useRouter();

  useEffect(() => { trackEvent('cle_mastery_checkout_view'); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError('');
    try {
      const res = await fetch('/api/orders/submit-cle-mastery', {
        method: 'POST',
      ...
      trackEvent('cle_mastery_order_submit', { orderId: data.orderId });
      router.push('/premium/cle-mastery/thank-you');
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-xl mx-auto">
        ...
        {/* Back link */}
        <Link href="/premium/cle-mastery" className="text-yellow-400 hover:text-yellow-300 text-sm mb-6 inline-block transition-colors">
          ← Back to product page
        </Link>

        {/* CLE Promo Counter */}
        <CLEPromoBar variant="compact" />
        ...
        <div className="bg-[#0f1629] border border-yellow-400/30 rounded-2xl p-6 mb-6">
          ...
          <p className="text-white font-bold mt-1">LisensyaPrep CLE Criminology Mastery System 2026</p>
        ...
        <div className="bg-[#0f1629] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <ol className="space-y-2">
            {[
              ...
              'You start studying for the CLE',
            ].map((item, i) => (
