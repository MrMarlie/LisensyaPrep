...
export default function AgriMasteryCheckoutPage() {
  const router = useRouter();

  useEffect(() => { trackEvent('agri_mastery_checkout_view'); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError('');
    try {
      const res = await fetch('/api/orders/submit-agri-mastery', {
        method: 'POST',
      ...
      trackEvent('agri_mastery_order_submit', { orderId: data.orderId });
      router.push('/premium/agri-mastery/thank-you');
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-xl mx-auto">
        ...
        {/* Back link */}
        <Link href="/premium/agri-mastery" className="text-green-400 hover:text-green-300 text-sm mb-6 inline-block transition-colors">
          ← Back to product page
        </Link>

        {/* Agri Promo Counter */}
        <AgriPromoBar variant="compact" />

        {/* Header */}
        <div className="bg-[#0f1629] border border-green-400/30 rounded-2xl p-6 mb-6">
          <p className="text-green-400 font-extrabold text-lg">Complete Your Order</p>
          <p className="text-white font-bold mt-1">LisensyaPrep Agriculture (ALE) Mastery System 2026</p>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-green-400 text-2xl font-extrabold">₱149</span>
            <span className="text-gray-500 line-through text-sm">₱249</span>
            <span className="text-xs bg-green-400/10 text-green-400 px-2 py-0.5 rounded-full font-semibold">Launch Special</span>
          </div>
        </div>

        {/* Step 1: GCash */}
        <div className="bg-[#0f1629] border border-green-400/30 rounded-2xl p-6 mb-6">
          <p className="text-green-400 font-extrabold mb-4">Step 1: Send Payment via GCash</p>
          <div className="flex flex-col sm:flex-row gap-6 items-start">
            <div className="flex-1">
              ...
              <div className="bg-green-400/10 border border-green-400/20 rounded-xl p-3">
                <p className="text-green-400 text-xs font-semibold">⚠️ Double-check the account name before sending!</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[#080d1b] border border-white/20 rounded-xl px-4 py-3 text-white text-base placeholder-gray-600 focus:outline-none focus:border-green-400/50 transition-colors"
                style={{ fontSize: '16px' }}
              />
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[#080d1b] border border-white/20 rounded-xl px-4 py-3 text-white text-base placeholder-gray-600 focus:outline-none focus:border-green-400/50 transition-colors font-mono tracking-widest"
                style={{ fontSize: '16px' }}
              />
            ...
            <div>
              ...
              <input
                ...
                max={today()}
                className="w-full bg-[#080d1b] border border-white/20 rounded-xl px-4 py-3 text-white text-base focus:outline-none focus:border-green-400/50 transition-colors"
                style={{ fontSize: '16px' }}
              />
              ...
              <div className="space-y-2">
                {REFERRAL_OPTIONS.map(({ value, label }) => (
                  <label key={value} className="flex items-center gap-3 cursor-pointer group">
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${form.referralSource === value ? 'border-green-400 bg-green-400' : 'border-white/30 group-hover:border-white/60'}`}>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-green-500 hover:bg-green-400 disabled:opacity-60 disabled:cursor-not-allowed text-white font-extrabold py-4 rounded-xl text-lg transition-colors mt-2"
            >
        ...
        <div className="bg-[#0f1629] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <ol className="space-y-2">
            {[
              ...
              'You start studying for the ALE',
            ].map((item, i) => (
        ...
        <div className="bg-[#0f1629] border border-white/10 rounded-2xl p-6 mb-6 text-sm text-gray-400">
          ...
          <p>Facebook: <a href="https://www.facebook.com/LisensyaPrep" target="_blank" rel="noopener noreferrer" className="text-green-400 hover:text-green-300">LisensyaPrep</a></p>
          <p>Email: <a href="mailto:lisensyaprep@gmail.com" className="text-green-400 hover:text-green-300">lisensyaprep@gmail.com</a></p>
