...
export default function CSEProMasteryCheckoutPage() {
  const router = useRouter();

  useEffect(() => { trackEvent('cse_pro_mastery_checkout_view'); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError('');
    try {
      const res = await fetch('/api/orders/submit-cse-pro-mastery', {
        method: 'POST',
      ...
      trackEvent('cse_pro_mastery_order_submit', { orderId: data.orderId });
      router.push('/premium/cse-pro-mastery/thank-you');
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-xl mx-auto">
        ...
        {/* Back link */}
        <Link href="/premium/cse-pro-mastery" className="text-yellow-400 hover:text-yellow-300 text-sm mb-6 inline-block transition-colors">
          ← Back to product page
        </Link>

        {/* CSE Promo Counter */}
        <CSEPromoBar level="pro" variant="compact" />
        ...
        <div className="bg-[#0f1629] border border-yellow-400/30 rounded-2xl p-6 mb-6">
          ...
          <p className="text-white font-bold mt-1">LisensyaPrep CSE Professional Mastery System 2026</p>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-yellow-400 text-2xl font-extrabold">₱199</span>
        ...
        <div className="bg-[#0f1629] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <ol className="space-y-2">
            {[
              ...
              'You start studying for the CSE Professional',
            ].map((item, i) => (
