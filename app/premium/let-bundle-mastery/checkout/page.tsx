...
export default function BundleCheckoutPage() {
  ...
  useEffect(() => { trackEvent('bundle_mastery_buy_click'); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError('');
    try {
      const res = await fetch('/api/orders/submit-bundle', {
        method: 'POST',
      ...
      router.push('/premium/let-bundle-mastery/thank-you');
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-xl mx-auto">
        ...
        <First100StatusBar variant="compact" bundleOverride />

        <Link href="/premium/let-bundle-mastery" className="text-yellow-400 hover:text-yellow-300 text-sm mb-6 inline-block transition-colors">
          ← Back to Bundle page
        </Link>

        {/* Header */}
        <div className="bg-[#0f1629] border border-green-400/20 rounded-2xl p-6 mb-6">
          <p className="text-green-400 font-extrabold text-lg">🎁 Complete Your Bundle Order</p>
          <p className="text-white font-bold mt-1">LET Mastery Bundle — ProfEd + Gen Ed Mastery Systems</p>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-yellow-400 text-2xl font-extrabold">₱399</span>
            <span className="text-gray-500 line-through text-sm">₱498</span>
            <span className="text-xs bg-green-400/10 text-green-400 px-2 py-0.5 rounded-full font-semibold">Save ₱99</span>
          </div>
          <p className="text-gray-400 text-xs mt-2">Includes: ProfEd Mastery PDF + Gen Ed Mastery PDF (both delivered to your email)</p>
        </div>
        ...
        <div className="bg-[#0f1629] border border-yellow-400/30 rounded-2xl p-6 mb-6">
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="bg-[#080d1b] rounded-xl p-4 border border-white/10">
              ...
              <p className="text-yellow-400 font-extrabold text-xl">₱399</p>
            </div>
          </div>
        ...
        <div className="bg-[#0f1629] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs mt-1">We&apos;ll send both PDFs here. Double-check!</p>
            </div>
            ...
            >
              {submitting ? 'Submitting...' : 'SUBMIT BUNDLE ORDER — ₱399'}
            </button>
          </form>
        </div>
        ...
        <div className="bg-[#0f1629] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <ol className="space-y-2">
            {[
              ...
              'We email you BOTH PDF reviewers via Google Drive links',
              'You start studying both reviewers',
            ].map((item, i) => (
