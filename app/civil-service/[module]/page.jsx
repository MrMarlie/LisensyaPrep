...
export async function generateStaticParams() {
  return Object.keys(MODULE_INFO)
    .filter((id) => MODULE_INFO[id].examId === 'civil-service')
    .map((id) => ({ module: id }));
}
...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    title: `${mod.title} — Civil Service Exam (CSE) Review`,
    description: mod.description,
    path: `/civil-service/${mod.id}`,
  });
}
...
export default function ModulePage({ params }) {
  const mod = MODULE_INFO[params.module];
  if (!mod || mod.examId !== 'civil-service') return notFound();
      ...
      <section className={`relative bg-gradient-to-b ${mod.color} py-16 overflow-hidden`}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <nav className="flex items-center gap-2 text-sm mb-6" aria-label="Breadcrumb">
            ...
            <Link href="/civil-service" className="text-gray-500 hover:text-gray-300">Civil Service (CSE)</Link>
            <span className="text-gray-700">/</span>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-6">
            ...
            <div>
              <p className="text-gray-400 text-sm font-semibold uppercase tracking-widest mb-1">
                Civil Service (CSE) • Stage {mod.stageNumber}
              </p>
