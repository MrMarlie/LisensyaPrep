...
export const metadata = PAGE_SEO.medicalTechnology;

const EXAM_INFO = [
  { label: 'Board Exam Name', value: 'Medical Technologist Licensure Examination (MTLE)' },
  { label: 'Conducted By', value: 'Professional Regulation Commission (PRC)' },
  { label: 'Schedule', value: 'Twice a year (usually April and October)' },
  { label: 'Passing Rate', value: '75% general weighted average required' },
  { label: 'Subjects Covered', value: 'Clinical Chemistry, Hematology, Microbiology & Parasitology, Immunology & Serology, Blood Banking, Urinalysis & Body Fluids' },
];
...
export default function MedicalTechnologyPage() {
  const modules = Object.values(MODULE_INFO).filter((m) => m.examId === 'medical-technology');

  return (
    <div className="min-h-screen">
      {/* Hero */}
      <section className="relative bg-gradient-to-b from-cyan-950/40 to-[#080d1b] py-16 sm:py-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-gray-300 text-sm">Medical Technology</span>
          </div>

          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-sm font-semibold px-4 py-2 rounded-full mb-5">
              🧪 Medical Technologist PRC Licensure Exam
            </div>
            <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4 leading-tight">
              Medical Technology PRC
              <br />
              <span className="text-cyan-400">Licensure Exam Review</span>
            </h1>
            <p className="text-gray-400 text-lg mb-8 max-w-2xl leading-relaxed">
              Battle through 6 modules covering the complete Medical Technologist Licensure Examination syllabus.
              Defeat the exam boss and collect your PRZ piece for each stage you complete.
            </p>

            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-cyan-400">✓</span> 6 Modules
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-cyan-400">✓</span> 300 Questions Total
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-cyan-400">✓</span> Detailed Explanations
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-300">
                <span className="text-cyan-400">✓</span> Progress Saved
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-14 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((mod) => (
            <Link key={mod.id} href={`/medical-technology/${mod.id}`} className="group">
      ...
      <section className="py-12 bg-[#0a1029]/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-extrabold text-white mb-6">About the Medical Technologist PRC Board Exam</h2>
