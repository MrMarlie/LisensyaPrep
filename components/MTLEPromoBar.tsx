...
export default function MTLEPromoBar({ variant }: { variant: 'full' | 'compact' }) {
  const [promo, setPromo] = useState<PromoData | null>(null);
  const [loading, setLoading] = useState(true);

  async function fetchPromo() {
    try {
      const res = await fetch('/api/promo/mtle-first-100');
      if (!res.ok) return;
  ...
  if (percent_filled <= 50) {
    spotText = `Only ${remaining} spots at this launch price!`;
    barClass = 'bg-cyan-400';
    wrapperClass = 'bg-[#001a1f] border-cyan-400/40';
    textClass = 'text-cyan-400';
  } else if (percent_filled <= 89) {
    spotText = `Only ${remaining} spots left — filling fast!`;
    barClass = 'bg-gradient-to-r from-cyan-400 to-sky-400';
    wrapperClass = 'bg-[#001520] border-sky-400/40';
    textClass = 'text-sky-400';
  } else {
    spotText = `🔥 LAST ${remaining} SPOTS at launch price!`;
    barClass = 'bg-red-500';
    ...
    return (
      <div className={`rounded-xl border px-4 py-3 mb-4 ${wrapperClass} ${pulse ? 'animate-pulse' : ''}`}>
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <p className={`font-bold text-sm ${textClass}`}>🧪 FIRST 100 BUYERS PROMO — {spotText}</p>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            <p className={`font-extrabold text-base ${textClass}`}>🧪 FIRST 100 BUYERS PROMO</p>
            <p className={`font-bold text-lg mt-0.5 ${textClass}`}>{spotText}</p>
            <p className="text-gray-300 text-sm mt-1">
              Launch price <strong className="text-white">₱{discounted_price}</strong>
              <span className="text-gray-500 line-through ml-2 text-xs">₱{regular_price}</span>
              <span className="text-gray-500 ml-1 text-xs">after promo</span>
            </p>
          ...
          <Link
            ...
            className="flex-shrink-0 bg-cyan-500 hover:bg-cyan-400 text-white font-extrabold px-6 py-3 rounded-xl text-base transition-colors shadow-lg whitespace-nowrap"
          >
