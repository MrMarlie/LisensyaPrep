...
export default function FreebieCTA({ redirectOnSuccess = false }: { redirectOnSuccess?: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setStatus('loading');
    trackEvent('freebie_signup_attempt', { source: pathname });

    try {
      const res = await fetch('/api/freebies/let-profed-starter-pack', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        setStatus('error');
        trackEvent('freebie_signup_failed', { source: pathname });
      } else {
        trackEvent('freebie_signup_success', { source: pathname });
        if (redirectOnSuccess) {
          router.push('/freebies/let-profed-starter-pack/success');
        } else {
          setStatus('success');
        }
      }
    ...
    return (
      <div className="my-8 rounded-2xl border border-green-400/40 bg-[#0f1629] p-6 text-center">
        ...
        <p className="text-gray-300 text-sm mb-1">
          Your LET ProfEd Starter Pack is on its way to <strong className="text-white">{email}</strong>.
        </p>
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          <p className="text-yellow-400 font-extrabold text-base uppercase tracking-wide">
            FREE: LET ProfEd Starter Pack
          </p>
          <p className="text-gray-300 text-sm mt-1">
            Get 30 LET ProfEd questions with full rationales, 8-week study schedule, and exam day
            checklist. 100% free — no credit card, no catch.
          </p>
        </div>
      </div>
