...
export default function CSESubProfFreebieCTA({ redirectOnSuccess = false }: { redirectOnSuccess?: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setStatus('loading');
    trackEvent('cse_subprof_freebie_signup_attempt', { source: pathname });

    try {
      const res = await fetch('/api/freebies/cse-subprof-starter-pack', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        trackEvent('cse_subprof_freebie_signup_success', { source: pathname });
        if (redirectOnSuccess) {
          router.push('/freebies/cse-subprof-starter-pack/success');
        } else {
          setStatus('success');
        }
      }
    ...
    return (
      <div className="my-8 rounded-2xl border border-green-400/40 bg-[#0f1629] p-6 text-center">
        ...
        <p className="text-gray-300 text-sm mb-1">
          Your CSE SubProfessional Starter Pack is on its way to <strong className="text-white">{email}</strong>.
        </p>
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          <p className="text-yellow-400 font-extrabold text-base uppercase tracking-wide">
            FREE: CSE SubProfessional Starter Pack
          </p>
          <p className="text-gray-300 text-sm mt-1">
            30 questions for first-level government positions — Verbal, Numerical, Clerical, and General Information.
            100% free, no catch.
          </p>
        </div>
      </div>
