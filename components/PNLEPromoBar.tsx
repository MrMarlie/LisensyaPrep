...
export default function PNLEPromoBar({ variant }: { variant: 'full' | 'compact' }) {
  const [promo, setPromo] = useState<PromoData | null>(null);
  const [loading, setLoading] = useState(true);

  async function fetchPromo() {
    try {
      const res = await fetch('/api/promo/pnle-first-100');
      if (!res.ok) return;
  ...
  if (percent_filled <= 50) {
    ...
    wrapperClass = 'bg-[#1a0010] border-pink-400/40';
    ...
    wrapperClass = 'bg-[#1a0015] border-rose-400/40';
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link
            ...
            className="flex-shrink-0 bg-pink-500 hover:bg-pink-400 text-white font-extrabold px-6 py-3 rounded-xl text-base transition-colors shadow-lg whitespace-nowrap"
          >
