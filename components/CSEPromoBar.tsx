...
const CONFIG = {
  subprof: {
    endpoint: '/api/promo/cse-subprof-first-100',
    checkout: '/premium/cse-subprof-mastery/checkout',
    emoji: '📋',
  },
  pro: {
    endpoint: '/api/promo/cse-pro-first-100',
    checkout: '/premium/cse-pro-mastery/checkout',
    emoji: '📊',
  },
} as const;

export default function CSEPromoBar({
  level,
  variant,
}: {
  level: 'subprof' | 'pro';
  ...
  if (percent_filled <= 50) {
    headerText = `${cfg.emoji} FIRST 100 BUYERS PROMO`;
    spotText = `Only ${remaining} spots left at this price!`;
    barClass = 'bg-yellow-400';
    wrapperClass = 'bg-[#16140250] border-yellow-400/40';
    ...
    barClass = 'bg-gradient-to-r from-yellow-400 to-amber-300';
    wrapperClass = 'bg-[#1a1503] border-amber-400/40';
