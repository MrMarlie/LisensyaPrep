...
export default function CSEProFreebieCTA({ redirectOnSuccess = false }: { redirectOnSuccess?: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setStatus('loading');
    trackEvent('cse_pro_freebie_signup_attempt', { source: pathname });

    try {
      const res = await fetch('/api/freebies/cse-pro-starter-pack', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        trackEvent('cse_pro_freebie_signup_success', { source: pathname });
        if (redirectOnSuccess) {
          router.push('/freebies/cse-pro-starter-pack/success');
        } else {
          setStatus('success');
        }
      }
    ...
    return (
      <div className="my-8 rounded-2xl border border-green-400/40 bg-[#0f1629] p-6 text-center">
        ...
        <p className="text-gray-300 text-sm mb-1">
          Your CSE Pro Starter Pack is on its way to <strong className="text-white">{email}</strong>.
        </p>
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          <p className="text-yellow-400 font-extrabold text-base uppercase tracking-wide">
            FREE: CSE Professional Starter Pack
          </p>
          <p className="text-gray-300 text-sm mt-1">
            30 questions across Verbal, Numerical, Analytical, and General Information — with full rationales.
            100% free, no catch.
          </p>
        </div>
      </div>
