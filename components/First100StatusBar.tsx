...
export default function First100StatusBar({ variant, bundleOverride = false }: Props) {
  const [promo, setPromo] = useState<PromoData | null>(null);
  const [loading, setLoading] = useState(true);

  async function fetchPromo() {
    try {
      const res = await fetch('/api/promo/let-first-100');
      if (!res.ok) return;
  ...
  if (percent_filled <= 50) {
    ...
    wrapperClass = 'bg-[#1a1400] border-yellow-400/40';
    ...
    wrapperClass = 'bg-[#1a0f00] border-orange-400/40';
  ...
  const ctaHref = bundleOverride
    ? '/premium/let-bundle-mastery/checkout'
    : '/premium/let-profed-mastery/checkout';
  const ctaText = bundleOverride
    ? 'GET THE BUNDLE — ₱399'
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-gray-300 text-sm mt-1">
              Save ₱{promo.total_slots - promo.claimed > 0 ? regular_price - discounted_price : 0} — launch price{' '}
          ...
          <Link
            href={ctaHref}
            className="flex-shrink-0 bg-yellow-400 hover:bg-yellow-300 text-gray-900 font-extrabold px-6 py-3 rounded-xl text-base transition-colors shadow-lg whitespace-nowrap"
          >
            {ctaText}
          </Link>
        </div>
